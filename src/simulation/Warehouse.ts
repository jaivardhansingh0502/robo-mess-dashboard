// ROBOMESS - Warehouse Topology & Grid Management (TypeScript)

import { SIMULATION_CONFIG, IStation, IRack, IDynamicObstacle, IPoint2D } from '../types/index.js';

export class Warehouse {
    public cols: number;
    public rows: number;
    public grid: number[][]; // 0 = walkable, 1 = static obstacle/rack, 2 = dynamic obstacle
    public racks: IRack[] = [];
    public stations: {
        inbound: IStation[];
        outbound: IStation[];
    ...
    public dynamicObstacles: Map<string, IDynamicObstacle> = new Map();

    constructor(cols: number = SIMULATION_CONFIG.GRID_COLS, rows: number = SIMULATION_CONFIG.GRID_ROWS) {
        this.cols = cols;
        this.rows = rows;
        this.grid = [];
        this.stations = {
    ...
    public addDynamicObstacle(x: number, y: number, label: string = 'Maintenance Barrier'): IDynamicObstacle | false {
        if (!this.isWithinBounds(x, y)) return false;
        const key = `${x},${y}`;
        this.grid[y][x] = 2;
        const obstacle: IDynamicObstacle = {
    ...
    public removeDynamicObstacle(x: number, y: number): boolean {
        const key = `${x},${y}`;
    ...
    public getClosestWalkableCell(x: number, y: number): IPoint2D {
    ...
    public getChargingStation(index: number = 0): IStation {
